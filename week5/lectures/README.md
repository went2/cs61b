...
不交集数据结构追踪的元素数量是固定的，一开始所有元素分别属于一个集合，简略起见，本文讨论的元素都是整型。不交集数据结构提供两个方法：
...
底层上仍用数组实现，且用索引号表示元素，用索引处的值表示该元素的父元素。以 0-5 这 6 个元素为例，初始化该结构，得到`[-1,-1,-1,-1,-1,-1]`，值 `-1` 表示这个元素为数的根节点，即表示一个独立的树。
...
衡量两个树的大小的规则是树的节点数，树A 包含的元素数量比树B 多，就说树A更大。数的大小保存到树根上，原来树根保存 -1，现在保存 `-(size of tree)`。
