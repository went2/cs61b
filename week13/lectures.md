...
实现快速排序时，要考虑细节上的问题，如主函数 quicksort 返回什么？分割函数 partition 返回什么？在我的实现中，主函数 quicksort 返回的是排序后的数组，所以要递归调用主函数；主函数需要知道分割后的数组以及 pivot 的位置，这样才能继续左右两个子数组的分割，所以分割函数会返回一个对象 `{ arr: number[], pivot-index: number }`，保存了排序后的数组以及 pivot 索引。

至于分割算法，先按自己的想法实现了一遍，又将下面提到的 Tony Hoare 发明的分割算法实现了一下，实现语言是 TypeScript，这是回过头来复习时实现的，代码见 `./quick-sort.ts`
